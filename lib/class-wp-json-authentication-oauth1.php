...
class WP_JSON_Authentication_OAuth1 extends WP_JSON_Authentication
{
    ...
    protected function check_oauth_signature($consumer, $oauth_params, $token = null)
    {
        ...
        // Need to get post_meta to match our current limitations.
        $key_parts = array( get_post_meta($consumer->ID, 'secret', true) );
        // Don't force the array, otherwise you get a random ampersand added to the key.
        if( $token ){
          $key_parts[] = $token->secret;
        }
